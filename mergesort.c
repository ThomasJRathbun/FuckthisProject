...
void printData( node * head, int _numHeaders, int socket)
{
	  node * curr = head;
	  int i = 0;
	  int l = 0;
	  ...
	  char size[6];
	  char temp[6];
	  buff = (char*)malloc(sizeof(char)*99999);
	  ...
	  while( curr != NULL)
		...
		written = 0;
	      ...
	      for ( i =0; i< 27;i++)
	      {
	    	  strcat(buff, curr->data[i]);
	    	  strcat(buff,",\0");
	      }
	      strcat(buff, curr->data[27]);
	      strcat(buff,"\0)");

	      memset(size,0,6);
	      memset(temp, 0, 6);
	      len = strlen(buff);
	      snprintf(temp, 10, "%d", len);
	      memcpy(size+(5-strlen(temp)),temp,strlen(temp) );
	      while( written != 5 )
	      {
		 written = write(socket, size+written,5-written);
              }
	      written = 0;
	      while (written != len)
	      {
	    	 written = write(socket, buff+written,len-written);
	      }
